...
const Do = (props: any) => {
  ...
  const refresh = useSyncCallback(() => {
    let netdisk_path = localStorage.getItem('netdisk_path')
    if(netdisk_path){
      localStorage.setItem('netdisk_path','')
      history.push(location.pathname + netdisk_path)
      return
    }
    if (!settingLoaded) {
      return;
    ...
    path().then((resp) => {
      ...
      if (res.code === 200) {
        ...
        if (res.code === 1001) {
          ...
          history.push(localStorage.getItem("DEPLOY_PATH") + "@manage");
        }
        if (res.code === 401) {
